...
import com.alura.foro.forohub.domain.respuesta.*;
import com.alura.foro.forohub.domain.respuesta.validaciones.actualizar.ValidarRespuestaActualizada;
import com.alura.foro.forohub.domain.respuesta.validaciones.crear.ValidarRespuestaCreada;
...
public class ControllerRespuesta {
    ...
    public ResponseEntity<DTODetalleRespuesta> crearRespuesta(@RequestBody @Valid CrearRespuestaDTO crearRespuestaDTO, UriComponentsBuilder uriBuilder){
        ...
        return ResponseEntity.created(uri).body(new DTODetalleRespuesta(respuesta));
    ...
    public ResponseEntity<Page<DTODetalleRespuesta>> leerRespuestaDeTopico(@PageableDefault(size = 5, sort = {"ultimaActualizacion"}, direction = Direction.ASC)Pageable pageable, @PathVariable Long topicoId){
        var pagina = respuestaRepository.findAllByTopicoId(topicoId, pageable).map(DTODetalleRespuesta::new);
       return ResponseEntity.ok(pagina);
    }

    @GetMapping("/usuario/{nombreUsuario}")
    @Operation(summary = "Lee todas las respuestas del nombre de usuario proporcionado.")
    public ResponseEntity<Page<DTODetalleRespuesta>> leerRespuestasDeUsuarios(@PageableDefault(size = 5, sort = {"ultimaActualizacion"}, direction = Direction.ASC) Pageable pageable, @PathVariable Long usuarioId){
        var pagina = respuestaRepository.findAllByUsuarioId(usuarioId, pageable).map(DTODetalleRespuesta::new);
        return ResponseEntity.ok(pagina);
    }

    @GetMapping("/{id}")
    @Operation(summary = "Lee una única respuesta por su ID")
    public ResponseEntity<DTODetalleRespuesta> leerUnaRespuesta(@PathVariable Long id){
        Respuesta respuesta = respuestaRepository.getReferenceById(id);

        var datosRespuesta = new DTODetalleRespuesta(
                respuesta.getId(),
    ...
    public ResponseEntity<DTODetalleRespuesta> actualizarRespuesta(@RequestBody @Valid DTOActualizarRespuesta actualizarRespuestaDTO, @PathVariable Long id){
